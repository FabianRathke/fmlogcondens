...
The drawback of this approach, is that these install binaries that where compiled for generic processors, which exclude AVX optimizations. See the next section, on how to activate AVX. 

### With AVX support (Linux and MacOS only)

AVX is the successor to SSE and MMX, and denotes the ability of a modern CPU to do certain simple computations (addition, multiplication, ...) in parallel on each core (not to be confused with the ability to split workloads across many cores). To activate AVX, we will install the package from source. To do this, we first download the source code from CRAN: https://cran.r-project.org/web/packages/fmlogcondens/index.html. Then from within R we run the command:

```{r}
install.packages("path-to-package/fmlogcondens_0.1.0.tar.gz", configure.args = "native", type = "source")
```
